...
import { localPathForUrl, normalizeUrl, relativeReference } from "../src/paths.js";
...
test("query strings create deterministic collision-resistant names", () => {
  const first = localPathForUrl("https://example.com/app.js?v=1", "text/javascript");
  const second = localPathForUrl("https://example.com/app.js?v=2", "text/javascript");
  assert.notEqual(first, second);
  assert.match(first, /app\.__q_[a-f0-9]{10}\.js$/);
});

test("normalization drops fragments and relative references are portable", () => {
  assert.equal(normalizeUrl("https://example.com/a#part"), "https://example.com/a");
  assert.equal(
    relativeReference(path.join("a", "pages", "index.html"), path.join("b", "app.js")),
    "../../b/app.js"
  );
});
